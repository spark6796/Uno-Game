...
from ..services.handler import game_handler_main
from ..config import normal_cards
import random 
from ..state import games 
from ..utils import generate_game_key
from ..models.game import PublicJoinRequest, JoinRequest, HostRequest
...
async def create_game(data: HostRequest):
    try:
...
async def public_join(data: PublicJoinRequest):
        session = SessionManager(data.session_id)
        ...
        all_game_ids = [game_id for game_id in games if games[game_id]['room_type'] == 'public' and len(games[game_id]['sessions']) != 4]
