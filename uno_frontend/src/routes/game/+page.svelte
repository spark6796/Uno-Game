...
<script>
    ...
    let im_leader = $state(false)
    let wild_color = $state()
    let previous_turn = $state('red')
    let new_turn = $state()
    let my_color = $state('red')
    let room_type = $state('private')
    let table_card = $state('uno.png')
    let current_card_combo = $state([])
    let special_cards = ['special/+4.png','special/wild.png']
    const api = import.meta.env.VITE_BACKEND_API
    const protocol = import.meta.env.VITE_SSL_BOOL == 'TRUE' ? 'wss' : 'ws';
    let global_layout = {}
    let my_party_players = $state({})
    let winner_pfp = $state()
    ...
    function game_manager(){
        ...
        let data = JSON.parse(event.data)
            if (data.action == "self_joined"){
                ...
                alert("Leader has left the party")
                goto('/')
            }
...
<div id="main" class="flex h-screen w-full bg-[#5472E4] overflow-hidden">   
    
    <div id="main_stuff" class="flex w-full rounded-lg opacity-0">
        
        <div class="flex flex-col gap-12 w-1/2 justify-center items-center">
            ...
            <button onclick={()=>start_game()} class="w-1/2 border-4 rounded-full border-blue-800 bg-blue-500 animate-pulse p-4 text-white text-4xl {im_leader ? '': 'hidden'}">START</button>
        </div>
        
        
        
        <div class="grid grid-cols-1 gap-y-4 gap-x-24 h-full w-1/2  border-blue-950 py-6">

            {#each Object.entries(my_party_players) as [name, pfp]}
                <div class="flex border-blue-800 items-center border-8 p-4 w-1/2 rounded-full h-fit bg-blue-500 mx-auto">
                    <img src={pfp} alt="pfp" class="border-4 border-blue-600 size-1/4 rounded-full">
...
<div id="game" class="max-h-screen h-screen max-w-full overflow-hidden bg-[#5472E4] hidden">
        ...
        <div class="flex justify-center items-center h-full w-full backdrop-blur-xl">
            <div class="flex flex-col h-1/2 w-1/3 bg-[#4e6de8] border-[#3a5bdda8] border-4 shadow-sm shadow-black p-6">
                ...
                <div class="grid grid-cols-2 grid-rows-2 w-full h-full px-12 gap-4">
                    <button aria-label="red" id="redButton" class="bg-red-400 hover:bg-red-500"></button>
                    <button aria-label="green" id="greenButton"  class="bg-green-400 hover:bg-green-500"></button>
                    <button aria-label="blue" id="blueButton" class="bg-blue-400 hover:bg-blue-500"></button>
                    <button aria-label="yellow" id="yellowButton" class="bg-yellow-400 hover:bg-yellow-500"></button>
                </div>
    ...
    <div class="absolute h-1/2 w-full">
        ...
        <div id="distributer" class="h-full w-full justify-center items-end hidden">
            <img alt="table_card" id='table_card' src="cards/uno.png" class="z-10 rounded-[2.9rem] h-2/3 w-[14%]">
            <img alt="distributer_card" id="distributer_card" src="cards/uno.png" class="absolute rounded-[2.9rem] h-2/3 w-[14%]">
    ...
    <div class="absolute bottom-8 h-1/2 w-full overflow-hidden">
        ...
        <div id="table" class="grid gap-2 grid-rows-3 grid-cols-10 w-1/2 h-full mx-auto rounded-xl border-8 bg-[#4e6de8] border-[#3a5bdda8] p-4 border-t-2 opacity-0">
            {#each players[my_color]['cards'] as card}
                    <button id={card} disabled={!((card.split('/').some(item => current_card_combo.includes(item)) || special_cards.includes(card)) && new_turn==my_color)} onclick={(event)=>throw_card_api(event)}><img class="rounded-2xl" alt="" src="cards/{card}"/></button>
            {/each}
        </div>
    </div>
</div>